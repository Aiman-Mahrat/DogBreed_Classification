...
def get_pet_labels(image_dir):
    ...
    for idx in range (0, len(filenames_list), 1):
        lower_pet_label = filenames_list[idx].lower()
        word_pet_label = lower_pet_label.split("_")
        pet_name=""
        for word in word_pet_label:
            if word.isalpha():
                pet_name += word + " "
    ...
    for idx in range(0, len(filenames_list), 1):
        if filenames_list[idx] not in results_dic:
             results_dic[filenames_list[idx]] = [pet_labels[idx]]
        else:
